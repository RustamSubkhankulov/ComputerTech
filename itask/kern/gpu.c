...
static void swap_displays(void);
...
static int gpu_set_display_vres(pair16_t vres)
{
    ...
    Vga_dev.display_coords[1].x = 0;
    Vga_dev.display_coords[1].y = Vga_dev.res.y;
    Vga_dev.display_offs[1] = vidmem_offset_by_coords(Vga_dev.display_coords[1], Vga_dev.res.x);

    Vga_dev.display_coords[0].x = 0;
    Vga_dev.display_coords[0].y = 0;
    Vga_dev.display_offs[0] = 0;

    Vga_dev.active_display = 0;
    Vga_dev.second_display = 1;
...
static void swap_displays(void)
{
    unsigned temp = Vga_dev.active_display;
    Vga_dev.active_display = Vga_dev.second_display;
    Vga_dev.second_display = temp;
}
...
int gpu_page_flip()
{
    ...
    if (Vga_dev.srfc_requested_cur == false)
    {
        vbe_dispi_set_reg(VBE_DISPI_INDEX_X_OFFSET, Vga_dev.display_coords[Vga_dev.second_display].x);
        vbe_dispi_set_reg(VBE_DISPI_INDEX_Y_OFFSET, Vga_dev.display_coords[Vga_dev.second_display].y);
    
        swap_displays();
    }

    Vga_dev.srfc_requested_cur = false;
...
int gpu_request_surface(srfc_t* surface, int flags)
{
    ...
    size_t offs = (flags & REQUEST_CUR)? Vga_dev.display_offs[Vga_dev.active_display] :
                                         Vga_dev.display_offs[Vga_dev.second_display] ;

    surface->buffer = Vga_dev.fb + offs;

    Vga_dev.srfc_requested_cur = (flags & REQUEST_CUR);
...
int gpu_submit_surface(const srfc_t* surface)
{
    ...
    size_t offs = (Vga_dev.srfc_requested_cur)? Vga_dev.display_offs[Vga_dev.active_display] : 
                                                Vga_dev.display_offs[Vga_dev.second_display] ;

    if (surface->buffer != Vga_dev.fb + offs)
        return -E_INVAL;
...
void test_gpu(void)
{
    ...
    {
        srfc_t surface = { 0 };
        int err = gpu_request_surface(&surface, 0);
        assert(err == 0);
